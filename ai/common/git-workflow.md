...
>
> 本文件**工具无关**（Claude Code / Codex / Antigravity 等通用），且**尽量机器无关**：可移植的方法见 §1–§3，某台机器的具体值见 §4「本机现状」。

`dut-institute-work` 是**研究院工作管理库**，且为 **Public 仓库、内容涉及单位内部工作**——提交纪律以"脱敏"为第一位，见 §5。

## §0 新机器启动语（复制即用）

**推荐做法：把本文件（git-workflow.md）发给新机器上的 agent，附这句话**（默认克隆本仓库，换别的仓库改路径即可）：

> 照这份 git-workflow.md（重点 §1、§3）把这台 Windows 电脑的 git 环境配好，用 PowerShell 原生 git（别用 cygwin/MSYS）。配到需要加公钥那步，把 id_ed25519.pub 打印给我、停下等我加到 GitHub。加好后把 git@github.com:brighthe/dut-institute-work.git（SSH）clone 到 C:\workspace，并 ls-remote 验证鉴权。

- 手上没有本文件、但有网时，可改让 agent 先读它的 raw 版：`https://raw.githubusercontent.com/brighthe/dut-institute-work/main/ai/common/git-workflow.md`。
...
- 远程用 SSH：`git@github.com:brighthe/dut-institute-work.git`。
...
- ⚠️ 绝对路径类配置**仅本机**有效；换机器一律照 §3 走。

## §5 提交纪律（本仓库 = Public 且涉及单位内部工作，脱敏优先）

- **仅在用户明确要求时**提交/推送。
- **脱敏是硬约束**：本仓库公开可见且涉及研究院内部工作。提交前**逐一 `git diff` 已暂存内容**，确认没有把账号、财务信息、合同金额、内部文件原文、未公开技术细节等推出去；必要时先脱敏或粗化再提交。具体红线见根目录 [README.md](../../README.md)「公开与脱敏纪律」。
- **绝不提交内部文件附件**（PDF、截图、表格原件）。`git status` 里出现附件时停下核对。
- 用 `git status` 甄别，**只 `git add` 本次任务相关文件**，不要 `git add -A`。
- 提交信息用**简体中文**，结尾附 `Co-Authored-By` 尾注。
- 在 `main` 上**直接提交，不开分支、不走 PR**。

## §6 排错速查

- `could not read Username for 'https://github.com'` → 远程是 HTTPS；`git remote set-url origin git@github.com:brighthe/dut-institute-work.git`。
